...
namespace RoutingConstraints {
	...
	public:
		...
		TEST_METHOD(TWCalculation) {
			...
			Assert::AreEqual(true, ConstraintsRouting::checkTimeWindowCustomer(1, 0, time, instance));
			Assert::AreEqual(expected, time);
			Assert::AreEqual(true, ConstraintsRouting::checkTimeWindows(solution, instance));
			...
			time = 0;
			Assert::AreEqual(true, ConstraintsRouting::checkTimeWindowCustomer(1, 0, time, instance));
			Assert::AreEqual(true, ConstraintsRouting::checkTimeWindowCustomer(2, 1, time, instance));
			Assert::AreEqual(expected, time);
		}
		...
		TEST_METHOD(ExceedingTW) {
			...
			// Test
			Assert::AreEqual(false, ConstraintsRouting::checkTimeWindows(solution, instance));
		}
	};
}