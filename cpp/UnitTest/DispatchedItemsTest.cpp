...
namespace RoutingConstraints {
	...
	public:
		TEST_METHOD(Demand2NotFulfilled) {
			...
			// Test
			Assert::AreEqual(false, ConstraintsRouting::checkDispatchedItems(solution, instance));
		}
		...
		TEST_METHOD(DemandFulfilled) {
			...
			// Test
			Assert::AreEqual(true, ConstraintsRouting::checkDispatchedItems(solution, instance));
		}
	};
}