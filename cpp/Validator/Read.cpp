...
Instance Read::readInstanceFile(std::string instancePath) {
	...
	ifstream inputFile(instancePath, ifstream::in);

	if (!inputFile) {
		cerr << "Could not open Instance file " << instancePath << endl;
		exit(-1);
	}
	...
	if (line != "CUSTOMERS") {
		cerr << "Wrong Line. Should be Header CUSTOMERS, current: " + line << endl;
	}
	getline(inputFile, line);
	...
	for (unsigned int i = 1; i <= no_customers; ++i) {
		...
		inputFile >> id;
		if (customer.id != id) {
			cerr << "Wrong Line. Expected: " << customer.id << ", read: " << id << endl;
		}
		for (unsigned int j = 0; j < totalDemand;) {
			pair<unsigned int, unsigned int> demand = readDemand(inputFile);
...
pair<unsigned int, unsigned int>  Read::readDemand(std::ifstream &inputFile) {
	string str;
	unsigned int qt;
	inputFile >> str >> qt;
	string no = str.substr(2);
	unsigned int id = stoi(no);
	return pair<unsigned int, unsigned int>(id, qt);
}


ConstraintSet Read::readConstraintFile(std::string constraintPath) {
	ifstream inputFile(constraintPath, ifstream::in);
	istringstream	iss;
	string			line, var;

	if (!inputFile) {
		cerr << "Could not open ConstraintSet file " << constraintPath << endl;
		exit(-1);
	}
...
Solution Read::readSolutionFile(const std::string solutionPath, Instance& instance) {
	ifstream inputFile(solutionPath, ifstream::in);
	istringstream	iss;
	string			line, var, name, problem, totalTravelDistance, calculationTime, iterations, cSet;

	if (!inputFile) {
		cerr << "Could not open Packplan file " << solutionPath << endl;
		exit(-1);
	}
	...
	if (name != instance.name) {
		cerr << "Wrong Packing Plan found. Expected: " << instance.name << ", read: " << line;
		exit(-1);
	}
	...
	for (size_t r = 1; r <= vUsed; ++r) {
		...
		vector<unsigned int> customerIds;
		...
		if (r != id) {
			cerr << "Wrong TourId found. Expected: " << r << ", read: " << id << endl;
			exit(-1);
		}
...
Item  Read::readItem(const string line, Instance& instance) {
	istringstream	iss;
	...
	ItemType itemtype = instance.itemTypes.at(type_id);
