...
public class Loading {
     ...
     */
    public static boolean checkLoadingConstraints(Solution solution, final ConstraintSet cSet, final Instance instance, final boolean msg) {
        ...
        for (Tour tour : solution.getTours()) {
            ...
            for (final int item_id : tour.getItem_ids()) {
                ...
                if (!checkVehicleWalls(currentItem, instance.getVehicle(), msg)
                        ...
                        || !checkAxleWeights(currentItem, tour, cSet.isAxle_weights(), instance, msg)
                        || !checkBalancedLoading(currentItem, tour, cSet.isBalanced_loading(), cSet.getBalanced_part(), instance, msg)
                        || !checkUnloadingSequence(currentItem, tour, endPos, cSet.getUSequence(), instance, msg)
                        || !checkReachability(currentItem, tour, cSet.getLambda() * scaleFactor, cSet.isReachability(), instance, msg)
                        || !checkVStability(currentItem, cSet.getVStability(), cSet.getAlpha(), instance, msg)
                        || !checkStacking(currentItem, tour, endPos, cSet.getStacking(), instance, msg)) {
                    if (msg) System.err.println("Solution is NOT feasible according Loading Constraints.");
                    tour.setFeasible(false);
                    return false;
                }
            }
        }
        if (msg) System.out.println("Solution is feasible according Loading Constraints.");
        return true;
    }
    ...
    public static void getRelevantItems(Tour tour, int tourEndPos, Instance instance) {
        ...
        for (int i = 0; i < tourEndPos; ++i) {
            ...
            var iter = current.getItemsBelow().iterator();
            while (iter.hasNext()) {
    ...
    public static boolean checkVStability(Item item, final VerticalStability vStability, final float alpha, Instance instance, final boolean msg) {
        ...
        switch (vStability) {
            ...
            case StaticStabilityRamos:  System.err.println("Constraint currently not implemented"); return true;
            case StaticStabilityMack:   return checkStaticsStabilityMack(item, alpha, instance, msg);
        }
        return true;
    }
     ...
     * @return feasibility of item position.
     */
    public static boolean checkFlyingItem(Item item, Instance instance, final boolean msg) {
        for (int belowId : item.getItemsBelow()) {
            Item below = instance.getItems().get(belowId);
            if (below.isBelow(item, true)) {
                return true;
            }
        }

        if (msg) System.err.println("Item " + item.getId() + " is flying.");
        return false;
    }
     ...
     * @return feasibility of item position.
     */
    public static boolean checkStaticsStabilityMack(Item item, final float alpha, Instance instance, final boolean msg) {
        boolean flagCG = false;
        double supportArea = 0;
	    final double centerGravityX = (0.5 * (item.getMax().getX() - item.getMin().getX())) + item.getMin().getX();
	    final double centerGravityY = (0.5 * (item.getMax().getY() - item.getMin().getY())) + item.getMin().getY();

        for (int otherId : item.getItemsBelow()) {
            Item other = instance.getItems().get(otherId);

            if (other.isBelow(item, false)) {
                // Calculate Support to Items at the Ground
                if (other.getMin().getZ() == 0) {
                    supportArea += calcSupportArea(item, other);
                }

                // Check Support of Center of Gravity
                if (other.isBelow(item, true)
                        && (centerGravityX >= other.getMin().getX()
                        && centerGravityX <= other.getMax().getX()
                        && centerGravityY >= other.getMin().getY()
                        && centerGravityY <= other.getMax().getY())) {
                    flagCG = true;
                }
            }
        }
        ...
        if (!flagCG) {
            if (msg) System.err.println("Item " + item.getId() + " center of gravity not supported.");
            return false;
        }

        final double support = supportArea / (item.getL() * item.getW());
        if (support < alpha) {
            if (msg) System.err.println("Item " + item.getId() + " not enough supported.");
            return false;
        }
